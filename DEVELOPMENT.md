...
- **坑：动效切图交付里两个"隐形失真"——动图帧被偷合并 + 改色 0 命中**（2026-09-24，362 人脸/声纹弹窗动图交付实战）

  **TL;DR**：① Pillow `save_all=True` 合成动图会**合并重复帧、改写时长**，并因 `ANMF flags=dispose:none` 让透明区残留上一帧；② 把素材"白色"当成 `#FFFFFF` 改色会 **0 命中**（实测该素材是米白 `#FCF9F7`）。

  - 问题：序列帧转 WebP 后用 Pillow 合成动图，**9 帧序列产出 8 帧、50 帧产出 41 帧**，时长被改写（读回全是 300ms）；解码回读与源帧逐像素比对差异 1222~17973 px。另一处：`--recolor FFFFFF=2E2D2D` 命中 0 像素，产出未改色的文件。
  - 根因：① libwebp `WebPAnimEncoder` 只保留"变化帧"，自动合并完全相同的连续帧并累加时长；② 它写出的 `ANMF` flags 是 `dispose=none`，而 libwebp 在 **no-blend 模式下会跳过 alpha==0 的像素** → 透明区保留上一帧画布（实测误差精确满足 `got_a = 2a - a²/255`，即与自身再混合一次）；③ 素材的"白色"是米白 `#FCF9F7` 而非纯白 —— 同批素材里**另一个组才是纯白**，逐组量色值才能发现。
  - 解决：动图改走**容器级 mux** —— 从单帧 webp 里抽出已编码的 VP8L chunk（**含 chunk 头**）塞进 `ANMF`，零重编码；flags 固定 `0x01` = dispose-to-background + no-blend（按 RFC 9649，bit0=disposal / bit1=blending）。**判据 = libwebp 解码回读逐字节一致**：实测 `0x00`（dispose-none）与"仅首帧 `0x02`"都 ❌，`0x01` ✅。改色则先量色值再写 FROM。
  - 预防：动图一律走 mux 脚本（内置结构层 + 解码层双重核验，全过才 `RESULT: OK`）；改色前用 `numpy.unique` 统计不透明区 RGB；**帧率也从同项目 mp4 反推**（`fps = mdhd.timescale / stts.delta`，30fps → 30000/1000；注意 `stts` 在 `moov>trak>mdia>minf>stbl>stts`，不在 `mdia` 直接子级）；帧时长按累计取整分配，别用固定 `33ms`（90 帧会变 2.97s）。
  - 沉淀：**png-sequence-to-webp** 技能（`convert_seq_to_webp.py` + `mux_anim_webp.py`）。
...
- **坑：Pillow 多尺寸 ICO 保存只写第一张，应用图标必须手动组装超椭圆 ICO**（2026-09-09，登录态切换器 app.ico 接入实战，沉淀为 superellipse-icon）——三条独立坑：①Pillow `img.save('.ico', sizes=..., append_images=...)` **只写第一张**（实测：调用后文件仅 739 字节、struct 读只有 1 个 16px 条目）→ 必须手写 `ICONDIR(6B) + 逐尺寸 ICONDIRENTRY(16B) + 各尺寸 PNG 条目`（见 make_icon.py `build_ico`，与 WindowTinter/app.ico 结构一致：6 条目/32bpp/PNG 压缩）；②掩膜不做超采样会锯齿硬边 → 掩膜 `ss=4` 超采样后 `LANCZOS` 降采样得抗锯齿软边；③源图 24bpp 无 alpha（方形底四角纯背景色）显示成方形 → 用超椭圆 alpha 掩膜（n=4 大尺寸 / n=8 小尺寸）裁角，不必先补 alpha 通道。方法论：**「API/库宣称支持多尺寸」≠「真写了多份」，产物务必用 struct 读 ICONDIR 逐条校验 6 条目/32bpp/PNG。**
