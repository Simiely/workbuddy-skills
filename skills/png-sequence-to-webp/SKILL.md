...
三组真实素材的基准读数，用来**快速判断"这组该给无损还是有损"**。注意是参照不是系数 —— 每套素材的颜色分布不同，**自己那组仍要按 A 主线第 2 步全量跑**。

| 案例 | 素材 | 源 PNG | 无损 m=6 exact | q90 | 结论 |
|---|---|---|---|---|---|
| A | 纯扁平色，188 张 212×212 | 1054.1 KB | **372.4 KB（35.3%）· 误差 0** | 反而更大 | 扁平 → 无损完胜 |
| B | 含软渐变填充，60 张 160×160 | 745.4 KB | 398.2 KB（53%） | **223.0 KB（30%）** | 渐变 → 有损才省得动 |
| C | 单色细线 + 大面积透明，90 张 1080×1080 | 1560.5 KB | **800.5 KB（51.3%）· 误差 0** | **1099.7 KB（70.5%）** | 单色 → 无损更小，有损是纯负收益 |

案例 B 拆开看更清楚 —— **有损的收益只来自渐变那一组**：

| 组 | 帧 | PNG | 无损 | q90 |
|---|---|---|---|---|
| 01_静帧 | 1 | 28.9K | 1.9K | 1.8K |
| 02_切换序列 | 9 | 97.2K | 21.9K | 21.5K |
| **03_循环帧（含蓝色渐变填充）** | 50 | 619.3K | **374.4K** | **199.8K** |
| 合计 | 60 | 745.4K | 398.2K（53%） | 223.0K（30%） |

案例 C 的有损档位（说明为什么"有损换体积"在单色素材上不成立）：

| 档位 | 体积 | 占 PNG | 不透明区 PSNR |
|---|---|---|---|
| 无损 m=6 exact | **800.5 KB** | 51.3% | 无差异（误差 0） |
| q95 | 1173.2 KB | 75.2% | 33.92 dB |
| q90 | 1099.7 KB | 70.5% | 33.88 dB |
| q80 | 1062.1 KB | 68.1% | 33.77 dB |

混合模式（扁平组无损 + 渐变组 q90）实测 **223.6K vs 全有损 223.0K** —— 体积几乎相同，但扁平部分保持 0 误差，**严格占优**。
