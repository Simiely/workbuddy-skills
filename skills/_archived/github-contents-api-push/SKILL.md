...
> ## ⚠️ 已归档（2026-09-03）
> 本技能已被 **`github-push-universal`**（skills/ 下）替代。新技能改用 **Python**（`push_repo.py`），能力更强：git 优先 → 失败**自动回退** Contents API（幂等对齐、PUT/DELETE、一致跳过），无需 Node 环境。
> 本目录保留完整历史正文供查阅（含 Node 版 `push_repo.js`、packed-refs 修正、Node 实现细节）。**新用请用 `skills/github-push-universal`；诊断连接问题用 `skills/github-connect-diag`。**
