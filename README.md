...
| **git-push-proxy-fix** Git 推送代理修复 | 诊断修复 "git push 反复失败但代理/Clash 测速正常"——根因多为 git 全局配置残留针对 github.com 的空代理覆盖（强制直连被墙）；含 PowerShell `curl -w` 假失败的验证避坑与成功判定 | [skills/git-push-proxy-fix/SKILL.md](skills/git-push-proxy-fix/SKILL.md) | v1 稳定 |
