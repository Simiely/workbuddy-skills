...
- 修订：**png-sequence-to-webp** 补回三组实测基准数据表（新增 `B7 实测基准数据`）——v1.9.0 三层重构时被压缩掉的案例 A/B/C 完整读数重新收入 B 组「按需查」：源 PNG / 无损 / q90 三列对照 + 案例 B 逐组拆解（收益只来自渐变那一组）+ 案例 C 四档位 PSNR。
  - 定位说明：主线保持精简（只留判定规则），数据挪到参照位，并明确「**是参照不是系数** —— 每套素材颜色分布不同，自己那组仍要按 A 主线第 2 步全量跑」。
  - 保留的关键读数：扁平色 188 张 1054.1K → 无损 372.4K（35.3%，误差 0）而 q80 反而更大；渐变组 619.3K → 无损 374.4K / q90 199.8K；单色细线 1560.5K → 无损 800.5K（51.3%）**小于** q90 的 1099.7K（70.5%）。
- 文档：CHANGELOG v1.9.1 / AGENTS 基线行更新（下一提交补 hash）。
